...
| (逗号前后两者等效) |                                 | Optional      |
+====================+=================================+===============+
| hpcConfig,hc       | hyperchain的配置文件路径，如    | Required      |
|                    | namespace.toml                  |               |
+--------------------+---------------------------------+---------------+
| fl                 | 清洗线下数据完                  | Required      |
| atoArchivePath,fap | 成后归档/快照数据的目标路径，如 |               |
|                    | “archive”                       |               |
+--------------------+---------------------------------+---------------+
| flatoConfig,fcp    | flato的配置文件目录，即flato安  | Required      |
|                    | 装包中的整个configuration目录； |               |
+--------------------+---------------------------------+---------------+
| namespace,ns       | hyperchain的分区名，如”global”  | Required      |
+--------------------+---------------------------------+---------------+
| flatoDBPath,fp     | 升级后flato的数据目录           | Optional      |
+--------------------+---------------------------------+---------------+
| onlineBlockDB,ob   | hyperchain线上区块数            | Optional      |
|                    | 据目录，如”data/filelog/block”  |               |
+--------------------+---------------------------------+---------------+
| onlineJournalDB,oj | hyperchain线上journal数据       | Optional      |
|                    | 目录，如”data/filelog/journal”  |               |
+--------------------+---------------------------------+---------------+
| onli               | hyperchain线上blockchain目      | Optional      |
| neBlockChainDB,obc | 录，如”data/leveldb/blockchain” |               |
+--------------------+---------------------------------+---------------+
| mqPath,mq          | hyperchain                      | Option        |
|                    | 线上MQ目录，如”data/leveldb/MQ” | al,仅当使用过 |
|                    |                                 | MQ功能时需要  |
+--------------------+---------------------------------+---------------+
| snapshotDBDir,sd   | hyperchain的快照db目录，如”d    | Optional      |
|                    | ata/leveldb/snapshots/“注意此处 |               |
|                    | 需要的不是单个snapshotdb的目录  |               |
+--------------------+---------------------------------+---------------+
| snapshotMeta,sm    | 记录快照的snapshot.meta         | Optional      |
+--------------------+---------------------------------+---------------+
| filterID,fid       | 快照的filter id                 | Optional      |
+--------------------+---------------------------------+---------------+
| archiveMeta,am     | 记录归档的archive.meta          | Optional      |
+--------------------+---------------------------------+---------------+
| offlineBlockDB,ofb | hyperchain线下区块数            | Optional      |
|                    | 据目录，如”data/filelog/backup” |               |
+--------------------+---------------------------------+---------------+
| of                 | hyperchain线下journal数据目录， | Optional      |
| flineJournalDB,ofj | 如”data/filelog/journal_backup” |               |
+--------------------+---------------------------------+---------------+
| of                 | hyperchain线下archiv            | Optional      |
| flineArchiveDB,ofa | e目录，如”data/leveldb/archive” |               |
+--------------------+---------------------------------+---------------+
| log                | 日志级别，可选                  | Optional      |
|                    | deb                             |               |
|                    | ug,info,notice,warning,error,cr |               |
|                    | itical,默认notice，大小写不敏感 |               |
+--------------------+---------------------------------+---------------+
